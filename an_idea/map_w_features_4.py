...
def get_biome_for_chunk_w_terrain_render(chunk_x, chunk_y, SaveFile):
...
def get_tile(x, y, SaveFile):
    if (x, y) not in SaveFile.terrain_memory:
        get_biome_for_chunk_w_terrain_render(x // CHUNK_SIZE, y // CHUNK_SIZE, SaveFile)
        #needs updating
    return SaveFile.terrain_memory.get((x, y)) # dont need to return memory bank because it follows up the chain
...
def get_terrain_tile_block(center_x, center_y, SaveFile):
    ...
    for j in range(center_y - CHUNK_SIZE // 2, center_y + CHUNK_SIZE // 2):
        ...
        for i in range(center_x - CHUNK_SIZE // 2, center_x + CHUNK_SIZE // 2):
            row.append(get_tile(i, j, SaveFile))
        output.append(row)
    return output
...
if __name__ == "__main__":
    ...
    tile_block = draw_biome_tiles(center_chunk_x, center_chunk_y)
    print(tile_block)
    tile_block = tile_block_insert_center_player(tile_block=tile_block)
    print(draw_tile_block(tile_block))

    print("\n🌍 Current Chunk View:")
    tile_block = draw_main_terrain(center_world_x,center_world_y)
    print(tile_block)
    tile_block = tile_block_insert_center_player(tile_block=tile_block)
    print(draw_tile_block(tile_block))
    save_memory()