...
BIOMES = {
    ...
    "desert": {"emoji": "🏜️ ", "tiles": [(0.3, "💧"), (0.5, "⛱️"), (0.75, "🏜️ "), (1.0, "🦂")]},
    "forest": {"emoji": "🌲", "tiles": [(0.3, "💧"), (0.5, "🌱"), (0.7, "🌲"), (1.0, "🌳")]},
    "snow": {"emoji": "❄️ ", "tiles": [(0.3, "💧"), (0.5, "🌫️ "), (0.7, "❄️ "), (1.0, "☃️ ")]}
}

FEATURE_CHANCES = {
    "🏠": 0.0003,  # 1.5% chance for a camp
    "🪨 ": 0.0007   # 2.5% chance for a rock
}

# Load memory from disk
def load_memory():
    global tile_memory, feature_memory
...
def save_memory():
    with open(SAVE_FILE, "wb") as f:
        pickle.dump({"tiles": tile_memory, "features": feature_memory}, f)
...
def get_chunk_dominant_biome(chunk_x, chunk_y):
    base = (chunk_x * CHUNK_SIZE, chunk_y * CHUNK_SIZE)
    if base not in tile_memory:
        return generate_chunk_data(chunk_x, chunk_y)
    return get_biome(*base)
