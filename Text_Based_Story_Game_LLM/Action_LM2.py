...
for i in ["Banjo","Bumpsh","Yam","boibest"]:
    character_list.append(Character(i))

char_context = "Characters:"
for i in character_list:
    char_context = char_context + f"\n{i.name}"
...
def If_Text_Different(examples_file,text):
    ...
    while On == True:
        ...
        #system prompt#short reply
        syt_prompt = "Decide if the effects of a log entry effects things in a different capacity than the log it came from. Simply response with Y or N"
        #syt_prompt = "you are to take a part of a narrative and add something interesting to that plot point."
        syt_prompt = [{"role": "user", "content": f"{syt_prompt}"}]
        messages_list = syt_prompt+messages_list
...
if __name__ == "__main__":
    ...
    Food_Catagory = If_Text("",text=name,SYT_PROMPT=syt_prompt[1],just_first_str=True)
    print(Food_Catagory)
    ...
    #double possitive or double negative depending on what side you want to air

    ratio = []
    for i in range(20):
        #if If_Text("name_item_consumed",log[2],SYT_PROMPT=sty_prompt[2],just_first_str=True):
        ratio.append(If_Text("",log[5],SYT_PROMPT=sty_prompt[4],non_linear=False))
    print(ratio)


    for i in range(2):
        character_list[index_character("Banjo",character_list)].damage(50)

    if If_Text("if_item_damage","Jerry punches a wall",SYT_PROMPT=if_damage_item):
        D_Test = If_Text("damage_item","Jerry Gets sick",SYT_PROMPT=Item_Damage)
